...
(() => {
  ...
  if (!searchBox.value) {
    variantCheckbox.checked =
      localStorage.getItem("variant") !== CHECKBOX_MANUALLY_DISABLED;
    sinonimCheckbox.checked =
      localStorage.getItem("sinonim") === CHECKBOX_MANUALLY_ENABLED;
    equivalentCheckbox.checked =
      localStorage.getItem("equivalent") === CHECKBOX_MANUALLY_ENABLED;
  ...
  const updateCheckboxState = (checkbox, key) => {
    localStorage.setItem(
      key,
      checkbox.checked ? CHECKBOX_MANUALLY_ENABLED : CHECKBOX_MANUALLY_DISABLED,
    );
  };
  variantCheckbox.addEventListener("change", () =>
    updateCheckboxState(variantCheckbox, "variant"),
  );
  sinonimCheckbox.addEventListener("change", () =>
    updateCheckboxState(sinonimCheckbox, "sinonim"),
  );
  equivalentCheckbox.addEventListener("change", () =>
    updateCheckboxState(equivalentCheckbox, "equivalent"),
  );
  ...
  pagerSelector.addEventListener("change", () => {
    ...
    localStorage.setItem(
      "mostra",
      pagerSelector.value === pagerSelector.dataset.default
        ? ""
        : pagerSelector.value,
    );
