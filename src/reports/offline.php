...
const IGNORED_HTTP_STATUS_CODES = ['200', '301', '302', '307', '308'];
...
function background_test_llibres_urls(): string
{
    ...
    foreach ($llibres as $llibre) {
        ...
        if (in_array($response_code, IGNORED_HTTP_STATUS_CODES, true)) {
            continue;
        }
...
function background_test_paremiotipus_repetits(int $start = 0, int $end = 0): string
{
    ...
    for ($i = $start; $i < $limit; $i++) {
        ...
        for ($u = $i + 1; $u < $total; $u++) {
            ...
            $output .= get_paremiotipus_display($value2, escape_html: false) . "\n\n";
        }
    }
...
function background_test_imatges_no_referenciades(): string
{
    ...
    foreach ($dir as $file_info) {
        ...
        if (!has_supported_image_extension($filename)) {
            continue;
        }
