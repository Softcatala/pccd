...
function test_dsff(): void
{
    ...
    $json_content = file_get_contents(__DIR__ . '/../../data/reports/dsff_v3beta.txt');
    ...
    echo ' El DSFF té actualment ' . format_nombre(count($data)) . ' frases, de les quals ' . format_nombre(count($sentences)) . ' no existeixen com a modisme a la PCCD.';
    ...
    echo '<h3>Frases del DSFF (2025) que existeixen com a modisme a la PCCD, però no amb la font comuna corresponent</h3>';
    ...
    echo '<h3>Modismes de la PCCD provinents del DSFF o de fonts comunes amb el DSFF però que no es troben com a frase al DSFF (2025)</h3>';
    echo "<i>A més dels motius anteriors, pot ser degut al fet que 1) el DSFF només incopori expressions lexicalitzades, no refranys ni dites. A que 2) el DSFF adapti i normalitzi la frase (p. ex. usant sempre el verb ésser, o amb les expressions indefinides usades entre parèntesis), o que 3) el buidatge a la PCCD també inclogués les variants formals i dialectals com a modisme. En el cas de la font DCVB, cal tenir en compte que la incorporació d'aquesta obra al DSFF no s'ha completat.</i>";
