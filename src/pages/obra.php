...
if ($obra->Observacions !== '') {
    ...
    $output .= '<dd property="description">' . html_escape_and_link_urls(prepare_text_for_html($obra->Observacions, escape_html: false)) . '</dd>';
    PageRenderer::setMetaDescription(prepare_text_for_html($obra->Observacions));
}
$output .= '</dl>';
...
$output .= ' a la base de dades, de les quals ' . format_nombre($collected_count);
$output .= ($collected_count === 1 ? ' està recollida' : ' estan recollides') . ' en aquest web.';
$output .= '</div>';
