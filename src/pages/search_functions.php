...
const PAGINATION_RESULTS_PER_PAGE = 10;
...
function get_search_mode(): SearchMode
{
    ...
    if (
        ...
        $trimmed_query = mb_trim($_GET['cerca']);
        if (str_starts_with($trimmed_query, '"') && str_ends_with($trimmed_query, '"')) {
            // Simple custom search mode for whole sentence search.
            return SearchMode::WHOLE_SENTENCE;
        }

        if (
            !str_contains($trimmed_query, ' ')
            && (
                str_contains($trimmed_query, '*')
                || str_contains($trimmed_query, '?')
            )
 ...
 */
function get_search_query_normalized(SearchMode $search_mode): string
{
    if (isset($_GET['cerca']) && $_GET['cerca'] !== '' && is_string($_GET['cerca'])) {
        $trimmed_query = mb_trim($_GET['cerca']);
        $query_length = strlen($trimmed_query);
        if ($query_length > 0 && $query_length < MAX_SEARCH_QUERY) {
            return normalize_search($trimmed_query, $search_mode);
        }
    }
...
function get_search_pagination_limit(): int
{
    ...
    return PAGINATION_RESULTS_PER_PAGE;
}
...
function get_search_pager_url(int $page_number): string
{
    ...
    if (!isset($_GET['cerca']) || $_GET['cerca'] === '' || !is_string($_GET['cerca'])) {
        ...
        if ($page_number === 1) {
            if ($results_per_page === PAGINATION_RESULTS_PER_PAGE) {
                return '/';
            }
