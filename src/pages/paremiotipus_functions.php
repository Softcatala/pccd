 ...
 */
function get_recurrences_by_variant(string $paremiotipus): array
{
 ...
 * @return list<Image>
 */
function get_paremiotipus_images(string $paremiotipus): array
{
...
/**
 * Renders a formatted citation from a paremiotipus recurrence.
 *
 * @param array<string, string> $editorials
 * @param array<string, string> $fonts
 */
function render_citation(ParemiotipusVariant $recurrence, array $editorials, array $fonts): string
{
    $citation = htmlspecialchars($recurrence->AUTOR);

    if ($recurrence->ANY !== '' && $recurrence->ANY !== '0') {
        $citation .= " ({$recurrence->ANY})";
    }

    // Add colon separator if we have author/year and additional content.
    if ($recurrence->DIARI !== '' || $recurrence->ARTICLE !== '') {
        $citation .= ':';
    }
...
/**
 * Renders the body content of a paremiotipus entry, including explanation, examples, synonyms, etc.
 */
function render_entry_body(ParemiotipusVariant $recurrence): string
{
    $body_html = '';

    // Build explanation from combined explanation fields.
    $explanation = mb_ucfirst(prepare_field($recurrence->EXPLICACIO . $recurrence->EXPLICACIO2));
    ...
    if ($recurrence->AUTORIA !== '') {
        $explanation .= ' De: ' . prepare_field($recurrence->AUTORIA);
    ...
    if ($recurrence->EXEMPLES !== '') {
        $body_html .= '<div><i>' . mb_ucfirst(prepare_field($recurrence->EXEMPLES)) . '</i></div>';
    }

    if ($recurrence->SINONIM !== '') {
        $body_html .= '<div>Sinònim: ' . prepare_field($recurrence->SINONIM) . '</div>';
    }

    if ($recurrence->EQUIVALENT !== '') {
        $equivalent = prepare_field($recurrence->EQUIVALENT);
    ...
    if ($recurrence->LLOC !== '') {
        $body_html .= '<div>Lloc: ' . prepare_field($recurrence->LLOC) . '</div>';
    }

    // Add source section (unless it is too short).
    if (strlen($recurrence->FONT) > 1) {
        $body_html .= '<div>Font: ' . prepare_field($recurrence->FONT) . '</div>';
    }
...
/**
 * Return the share icons HTML code.
 */
