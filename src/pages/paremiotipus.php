...
$paremiotipus_id = current($recurrences_by_variant)[0]->getId();
...
foreach ($recurrences_by_variant as $modisme => $recurrences) {
    ...
    foreach ($recurrences as $recurrence) {
        $year = $recurrence->getYear();
        if ($year > 0 && $year < $min_year_per_variant) {
            $min_year_per_variant = $year;
        }

        $body = $recurrence->renderBody();
        $citation = $recurrence->renderCitation($editorials, $fonts);
        if ($body !== '' || $citation !== '') {
            ...
            $current_source_id = $recurrence->getSourceId();
...
// Print multilingüe button if there are translations.
$translations_html = render_paremiotipus_translations($paremiotipus_id);
if ($translations_html !== '') {
    echo render_paremiotipus_translations_button();
}
// Print share icons.
...
// Print multilingüe content.
if ($translations_html !== '') {
    echo '<div class="multilingue visually-hidden">' . $translations_html . '</div>';
}
