...
final class ComposerPackageJsonTest extends TestCase
{
    ...
    public function testComposerPackageMatch(): void
    {
        ...
        foreach ($fields as $field) {
            self::assertSame($this->package[$field], $this->composer[$field], "Field {$field} in package.json and composer.json must match");
        }
    }

    private function getJsonArray(string $filename): array
    {
        return json_decode(file_get_contents($filename), associative: true, flags: JSON_THROW_ON_ERROR);
    }
}
