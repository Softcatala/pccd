...
test.describe("Homepage <picture> tag", () => {
  ...
  test("the picture includes an AVIF file", async ({ page }) => {
    const pictureSrcset = await page
      .locator("picture source")
      .getAttribute("srcset");
    expect(pictureSrcset).toMatch(/\.avif/);
  });

  test("the server sets the correct image/avif content type for the AVIF file", async ({
    page,
  }) => {
  ...
  test("all files in the picture element have the correct Cache-Control header", async ({
    page,
  }) => {
  ...
  test("all files in the picture element have the correct Strict-Transport-Security header", async ({
    page,
  }) => {
    const urls = await page.evaluate(() => {
      const sources = document.querySelectorAll("picture source");
  ...
  test("homepage includes a preloaded image with type and media attributes", async ({
    page,
  }) => {
...
test.describe("SVG in <img> tags", () => {
  ...
  test("the server sends correct type and sets Brotli compression for SVG files", async ({
    page,
  }) => {
    const svgUrls = await page.evaluate(() =>
      [...document.querySelectorAll('img[src$=".svg"]')].map((img) => img.src),
    );
