...
test.describe("Search", () => {
  ...
  test("wildcard search returns all results", async ({ page }) => {
    ...
    const resultats = await page
      .locator("form[role=search] > p")
      .first()
      .textContent();
  ...
  test(`"fera" returns ${data.searchFeraNumberOfResults} results`, async ({
    page,
  }) => {
  ...
  test(`"fera" with variants returns ${data.searchFeraWithVariantsNumberOfResults} results`, async ({
    page,
  }) => {
  ...
  test(`"Val més un boig conegut que un savi per conèixer" returns exactly 1 result`, async ({
    page,
  }) => {
    await page.goto(
