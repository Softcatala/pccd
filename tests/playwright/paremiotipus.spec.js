...
test.describe("Paremiotipus", () => {
  ...
  test(`"Qui no vulgui pols, que no vagi a l'era" has ${data.paremiotipusQuiNoVulguiPolsNumberOfEntries} records`, async ({
    ...
    const match = /(?<number>[\d.]+)\srecurrències/u.exec(results);
  ...
  test(`"Qui no vulgui pols, que no vagi a l'era" has ${data.paremiotipusQuiNoVulguiPolsNumberOfVariants} variants`, async ({
    ...
    const match = /en (?<number>[\d.]+)\svariants/u.exec(results);
    extractedNumber = match.groups.number;
  ...
  test(`"Val més un boig conegut que un savi per conèixer" has CV audio`, async ({
    page,
  }) => {
    await page.goto(
      "/p/Val_m%C3%A9s_un_boig_conegut_que_un_savi_per_con%C3%A8ixer",
    );
    const commonVoicePath = await page
      .locator("#commonvoice audio")
      .first()
      .getAttribute("src");
    expect(commonVoicePath).toContain(".mp3");
  });

  test(`"—Què hem de fer? —Vendre la casa i anar de lloguer" page has correct title`, async ({
    page,
  }) => {
    await page.goto(
      "/p/Qu%C3%A8_hem_de_fer%3F_%E2%80%94Vendre_la_casa_i_anar_de_lloguer",
    );
    const pageTitle = await page.title();
    expect(pageTitle).toBe(
      "—Què hem de fer? —Vendre la casa i anar de lloguer | PCCD",
    );
  });
});
