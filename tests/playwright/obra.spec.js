...
test.describe("Obra", () => {
  ...
  test(`"Folklore de Catalunya. Cançoner" has ${data.obraFolkloreCatalunyaNumberOfEntries} entries, of which
            ${data.obraFolkloreCatalunyaNumberOfEntriesCollected} has been collected`, async ({
    page,
  }) => {
    await page.goto(
    ...
    [, extractedNumber] =
      /Aquesta obra té ([\d.]+) fitxes a la base de dades/.exec(obra);

    expect(Number(extractedNumber.replace(".", ""))).toBe(
      data.obraFolkloreCatalunyaNumberOfEntries,
    );

    [, extractedNumber] = /de les quals ([\d.]+) estan recollides/.exec(obra);
    expect(Number(extractedNumber.replace(".", ""))).toBe(
      data.obraFolkloreCatalunyaNumberOfEntriesCollected,
    );
  });
