...
**Principle**: Pin versions as tightly as practical to ensure reproducible builds. Dependencies are actively kept at their latest stable release (or LTS for MariaDB) via `scripts/update_deps.sh`. Only well-established, actively maintained packages are accepted; abandoned or unmaintained dependencies should be replaced or removed.
