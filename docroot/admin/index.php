...
<head>
    ...
    <style>
        ...
        details, details summary { color: yellowgreen; cursor: pointer; }
        details * { color: #dbdbdb; cursor: auto; }
...
echo 'PHP ' . PHP_VERSION . (defined('PHP_BUILD_DATE') ? ' (' . PHP_BUILD_DATE . ')' : '') . ', ' . apache_get_version() . ' (' . PHP_OS . '), ' . $mysql_version;
...
if (function_exists('opcache_get_status')) {
    $status = opcache_get_status(false);
    if ($status !== false && isset($status['opcache_statistics']['hits'], $status['opcache_statistics']['misses'])) {
...
if (function_exists('apcu_sma_info')) {
    ...
    if (is_array($mem) && isset($mem['avail_mem'])) {
        assert(is_int($mem['avail_mem']));
        echo '<br>APCu memòria lliure: ' . round($mem['avail_mem'] / 1024 / 1024, 1) . ' MB';
    }
}
