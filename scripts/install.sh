...
if [ "${IS_DOCKER}" -eq 1 ]; then
  ...
  docker compose exec -T web php scripts/install/install.php
fi
...
if [ "${IS_DOCKER}" -eq 1 ]; then
  php install/build_sitemap.php > ../docroot/sitemap.txt
else
  docker compose exec -T web php scripts/install/build_sitemap.php > ../docroot/sitemap.txt
fi
...
node -e "console.log(new Intl.DateTimeFormat('ca-ES', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date()).replace(/’/g, \"'\").replace(/ del /g, ' de '));" > ../data/db_date.txt
