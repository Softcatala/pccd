...
compare_docker_tags() {
    ...
    digest1=$(docker pull "${IMAGE_NAME}:${TAG1}" | grep -o 'sha256:[a-f0-9]*' | head -1) || true
    if [[ -z "${digest1}" ]]; then
        echo "Failed to pull or extract digest from ${IMAGE_NAME}:${TAG1}"
        return 2
    fi

    # Pull second image
    echo "Pulling ${IMAGE_NAME}:${TAG2}..."
    digest2=$(docker pull "${IMAGE_NAME}:${TAG2}" | grep -o 'sha256:[a-f0-9]*' | head -1) || true
    if [[ -z "${digest2}" ]]; then
        echo "Failed to pull or extract digest from ${IMAGE_NAME}:${TAG2}"
        return 2
    fi
...
update_npm() {
    ...
    jq '.devDependencies | keys | .[]' package.json | xargs npm install --save-dev --save-exact --ignore-scripts
    jq '.dependencies | keys | .[]' package.json | xargs npm install --save --save-exact --ignore-scripts
