...
echo "DROP TABLE IF EXISTS RML;" >> ../install/db/db.sql
...
mdb-schema --no-indexes --no-relations -T RML ../"${DATABASE_FILE}" mysql >> ../install/db/db.sql
...
mdb-export -I mysql ../"${DATABASE_FILE}" "RML" >> ../install/db/db.sql
...
# Multilingüe.
echo "ALTER TABLE RML ADD INDEX (PAREMIOTIPUS);" >> ../install/db/db.sql
