...
sync_docker_compose_image_tag() {
  ...
  target_tag=$(grep -E "image:[[:space:]]*${IMAGE_NAME}:" "${TARGET_COMPOSE_FILE}" | head -1 | sed -E "s/.*image:[[:space:]]*${IMAGE_NAME}://" | tr -d ' ')
...
update_composer() {
  ...
  non_dev_tmp=$(mktemp)
  all_tmp=$(mktemp)
  ./composer.phar show --direct --no-dev --name-only | sort > "${non_dev_tmp}"
  ./composer.phar show --direct --name-only | sort > "${all_tmp}"
  grep -F -v -f "${non_dev_tmp}" "${all_tmp}" |
    xargs ./composer.phar require --dev --update-with-all-dependencies
  rm -f "${non_dev_tmp}" "${all_tmp}"
}
