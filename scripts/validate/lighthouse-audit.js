...
const auditUrl = async (url, baseUrl, device = "desktop") => {
  ...
  try {
      ...
      if (score !== PERFECT_SCORE) {
        ...
        if (
          (category.id === "performance" &&
            score > PERFORMANCE_SCORE_ACCEPTABLE) ||
          (category.id === "seo" &&
            score === SEO_SCORE_MISSING_META &&
            (url.startsWith(`${baseUrl}/p/`) ||
              url.startsWith(`${baseUrl}/obra/`))) ||
          (device === "small-mobile" &&
    ...
    if (allScoresPerfect) {
      console.log(
        `✅ All essential audits score 100% for ${url} on ${device}.`,
      );
    } else {
      console.error(
        `   Re-run with: npx lighthouse "${url}" --view --chrome-flags="--headless"`,
      );
