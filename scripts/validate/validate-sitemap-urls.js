 ...
 */
const productionToLocalUrl = (url, baseUrl) =>
  url.replace("https://pccd.dites.cat", baseUrl);

/**
 * Converts a local URL to a production URL.
 */
const localToProductionUrl = (url, baseUrl) =>
  url.replace(baseUrl, "https://pccd.dites.cat");
...
const validateHtmlUrl = async (url, baseUrl, reportFiles) => {
  ...
  if (htmlhintResults.length > 0) {
    const errors = htmlhintResults
      .map(
        (result) =>
          `line ${result.line}, col ${result.col}, ${result.message} (${result.rule.id})`,
      )
      .join("\n");
    await appendFile(
      htmlErrorsFile,
  ...
  if (!htmlValidateResult.valid) {
    const errors = htmlValidateResult.results[0].messages
      .map(
        (message) =>
          `${filename}:${message.line}:${message.column}: ${message.message} (${message.ruleId})`,
      )
      .join("\n");
    await appendFile(
      htmlErrorsFile,
  ...
  if (html.includes('<div class="summary">')) {
    await appendFile(
      zeroFontsFile,
      `${localToProductionUrl(localUrl, baseUrl)}\n`,
    );
  }
};
...
const processUrlsBatch = async (urls, baseUrl, options) => {
  ...
  for (let index = 0; index < urls.length; index += concurrency) {
    ...
    await Promise.all(
      batch.map((url) => validateHtmlUrl(url, baseUrl, reportFiles)),
    );
  }
};
...
const htmlErrorsFile = path.join(rootDirectory, "tmp/test_html_errors.txt");
const zeroFontsFile = path.join(rootDirectory, "tmp/test_zero_fonts.txt");
...
const sitemapContent = await readFile(
  path.join(rootDirectory, "docroot/sitemap.txt"),
  "utf8",
);
...
const temporaryFiles = await readdir(path.join(rootDirectory, "tmp"));
await Promise.all(
  temporaryFiles
    .filter((file) => file.endsWith(".html"))
    .map((file) => unlink(path.join(rootDirectory, "tmp", file))),
);
