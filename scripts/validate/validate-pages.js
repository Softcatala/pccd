 ...
 */
const validateUrl = async (url, baseUrl, options = {}) => {
  const {
    skipTidy = false,
    skipHtmlhint = false,
    skipLinkinator = false,
    skipHtmlvalidate = false,
    checkExternal = false,
  } = options;
  ...
  if (!skipLinkinator) {
    console.log("==============");
    console.log("linkinator");
    console.log("==============");
    const linkinatorOptions = {
      path: url,
    };
    if (checkExternal) {
      console.log("Checking both internal and external links.");
    } else {
      console.log("Checking internal links only.");
      linkinatorOptions.linksToSkip = [new RegExp(`^(?!${baseUrl})`, "u")];
    }
    const linkinatorResult = await check(linkinatorOptions);
    const brokenLinks = linkinatorResult.links.filter(
  ...
  if (!skipHtmlvalidate) {
    ...
    if (!htmlValidateResult.valid) {
      for (const message of htmlValidateResult.results[0].messages) {
        console.error(
          `${outputFilename}:${message.line}:${message.column}: ${message.message} (${message.ruleId})`,
        );
      }
      process.exit(EXIT_CODE_ERROR);
    }
...
// Validate HTML
await validateUrl(`${process.env.BASE_URL}/`, process.env.BASE_URL);
await validateUrl(`${process.env.BASE_URL}/projecte`, process.env.BASE_URL);
await validateUrl(`${process.env.BASE_URL}/top100`, process.env.BASE_URL);
await validateUrl(`${process.env.BASE_URL}/llibres`, process.env.BASE_URL);
await validateUrl(`${process.env.BASE_URL}/instruccions`, process.env.BASE_URL);
await validateUrl(`${process.env.BASE_URL}/credits`, process.env.BASE_URL);
await validateUrl(`${process.env.BASE_URL}/fonts`, process.env.BASE_URL, {
  skipLinkinator: true,
});
await validateUrl(
  `${process.env.BASE_URL}/p/A_Abrera%2C_donen_garses_per_perdius`,
...
await validateUrl(
  `${process.env.BASE_URL}/obra/Pons_Lluch%2C_Josep_%281993%29%3A_Refranyer_menorqu%C3%AD`,
  process.env.BASE_URL,
);
await validateUrl(`${process.env.BASE_URL}/?pagina=5147`, process.env.BASE_URL);
await validateUrl(
  `${process.env.BASE_URL}/?mode=&cerca=ca%C3%A7a&variant=&mostra=10`,
  process.env.BASE_URL,
);
await validateUrl(
  `${process.env.BASE_URL}/p/A_Adra%C3%A9n%2C_tanys`,
  process.env.BASE_URL,
);
await validateUrl(
  `${process.env.BASE_URL}/p/A_Alaior%2C_mostren_la_panxa_per_un_guix%C3%B3_o_bot%C3%B3`,
  process.env.BASE_URL,
);
await validateUrl(`${process.env.BASE_URL}/?mostra=-1`, process.env.BASE_URL, {
  skipLinkinator: true,
