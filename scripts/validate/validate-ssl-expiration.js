...
const usage = () => {
  ...
  console.log(
    "  ENVIRONMENT_URL       The website URL, without trailing slash (default: https://pccd.dites.cat)",
  );
  console.log(
...
const expirationDate = await getCertificateExpiration(
...
const expirationDateDays = Math.floor(
  (expirationDate - currentDate) / DAYS_IN_MILLISECONDS,
);
...
const location = originIp
  ? `${remoteEnvironmentUrl} (${originIp})`
  : remoteEnvironmentUrl;
console.log(
  `${color}${location} certificate expires in ${expirationDateDays} days.${nc}`,
);
