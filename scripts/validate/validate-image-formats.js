...
const outputExtensionsFile = path.join(rootDirectory, "tmp/test_imatges_extensions.txt");
const outputFormatFile = path.join(rootDirectory, "tmp/test_imatges_format.txt");
...
const checkImageIntegrity = async (category) => {
  ...
  try {
    const { stdout } = await execAsync(`${PNGCHECK} "${relativeCategoryPath}"/*.png | ${GREP} -v 'OK:'`, {
      cwd: rootDirectory,
    });
  ...
  try {
    const { stderr } = await execAsync(`${GIFSICLE} --info "${relativeCategoryPath}"/*.gif`, { cwd: rootDirectory });
...
await Promise.all([writeFile(outputExtensionsFile, extensionsContent), writeFile(outputFormatFile, formatContent)]);
