...
const paremiesTargetDirectory = path.join(
  import.meta.dirname,
  "../../docroot/img/imatges",
);
...
const cobertesTargetDirectory = path.join(
  import.meta.dirname,
  "../../docroot/img/obres",
);
...
const fileSizeDifferenceBelowThreshold = function (
  file1,
  file2,
...
const resizeImage = async function (sourceFile, targetFile, width) {
  ...
  if (
    ...
    fileSizeDifferenceBelowThreshold(sourceFile, targetFile)
  ) {
...
const createAvifImage = function (sourceFile, targetFile, width) {
  ...
  sharp(sourceFile)
    ...
    .catch((error) => {
      console.error(
        `Error while processing ${sourceFile} to AVIF: ${error.message}`,
      );
    });
};

const processPng = async function (sourceFile, targetFile, width) {
...
const processJpg = async function (sourceFile, targetFile, width) {
  await resizeImage(sourceFile, targetFile, width);
...
const resizeAndOptimizeImagesBulk = async function (
  sourceDirectory,
  targetDirectory,
  width,
) {
...
const deleteUnusedImages = function (targetDirectory, sourceDirectory) {
  ...
  for (const targetFile of targetFiles) {
    const targetFileBaseName = path.basename(
      targetFile,
...
const main = async function () {
  await resizeAndOptimizeImagesBulk(
    cobertesDirectory,
    cobertesTargetDirectory,
    IMAGE_WIDTH,
  );
  await resizeAndOptimizeImagesBulk(
    paremiesDirectory,
    paremiesTargetDirectory,
    IMAGE_WIDTH,
  );
